...
The open62541 Client SDK option was added later, in 2024.
The client library is not as complete and robust as the UASDK,
we have seen the API change over recent years.

However, as the open62541 project is getting more mature and stable,
an increasing number of Device Support users have switched to this option.

## Status

Despite of being relatively new,
this option is gaining popularity
...
See the EPICS [Windows Installation How-To][windows-howto]
for an overview and introduction to the different compilation options
under Windows.
...
that sets the absolute path of your Open62541 installation
as well as its build and deploy features if necessary.
...
you need to set the location of the MinGW64 installation
in a CONFIG_SITE file, e.g.
...
When using MSVC,
you need to add the settings for the dependencies
that you installed in non-system locations:
...
the opcua module, the open62541 library and all other dependencies you installed.
Alternatively,
...
The adaption of the open62541 client library
has been done by Dirk Zimoch (PSI)
with help from Carsten Winkler (HZB/BESSY II).
Additional EPICS build system magic by Ralph Lange (ITER).

<!-- Links -->

[windows-howto]: https://docs.epics-controls.org/en/latest/getting-started/installation-windows.html
